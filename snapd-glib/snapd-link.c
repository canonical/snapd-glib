 ...
 */
  ...
  *
  * A #SnapdLink represents a link (contact, issues etc) that are associated
  * with a snap. Snap link can be queried using snapd_snap_get_links().
  */
 ...
 SnapdLink *snapd_link_new(void) {
   return g_object_new(SNAPD_TYPE_LINK, NULL);
 }
 ...
 static void snapd_link_class_init(SnapdLinkClass *klass) {
   ...
   g_object_class_install_property(
       ...
       g_param_spec_boxed("urls", "urls", "URLs available for this type of link", G_TYPE_PTR_ARRAY,
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY |
                               G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK |
                               G_PARAM_STATIC_BLURB));
 }
