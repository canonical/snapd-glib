...
static GPtrArray *
_snapd_json_parse_connections (JsonObject *object, const gchar *name, GError **error)
{
    g_autoptr(JsonArray) array = NULL;
...
static GHashTable *
_snapd_json_parse_attributes (JsonObject *object, const gchar *name, GError **error)
{
    JsonObject *attrs;
    JsonObjectIter iter;
...
SnapdPlug *
_snapd_json_parse_plug (JsonObject  *object,
                        GError     **error)
{
    g_autoptr(GPtrArray) connections = NULL;
    g_autoptr(GHashTable) attributes = NULL;

    connections = _snapd_json_parse_connections (object, "slot", error);
    if (connections == NULL)
        return NULL;
    attributes = _snapd_json_parse_attributes (object, "slot", error);
    if (attributes == NULL)
        return NULL;
                        ...
                        GError     **error)
{
    ...
    connections = _snapd_json_parse_connections (object, "plug", error);
    if (connections == NULL)
        return NULL;
    attributes = _snapd_json_parse_attributes (object, "plug", error);
    if (attributes == NULL)
        return NULL;
