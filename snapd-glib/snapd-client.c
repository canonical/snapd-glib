...
static RequestData *request_data_ref(RequestData *data) {
  data->ref_count++;
  return data;
}

static void request_data_unref(RequestData *data) {
  data->ref_count--;
  if (data->ref_count > 0)
    return;
...
static RequestData *get_request_data(SnapdClient *self, SnapdRequest *request) {
...
static gboolean async_poll_cb(gpointer data) {
  ...
  g_autoptr(SnapdGetChange) change_request =
      _snapd_request_async_make_get_change_request(
          SNAPD_REQUEST_ASYNC(d->request));
...
static void schedule_poll(SnapdClient *self, SnapdRequestAsync *request) {
...
static gchar *posix_lang_to_rfc2616(const gchar *language) {
  /* Don't include charset variants, etc */
  if (strchr(language, '.') || strchr(language, '@'))
    return NULL;

  /* Ignore "C" locale, which g_get_language_names() always includes as a
   * fallback. */
...
static gchar *add_quality_value(const gchar *str, int quality) {
...
/* Copied from libsoup */
static gchar *get_accept_languages(void) {
...
static SnapdPostChange *find_post_change_request(SnapdClient *self,
  ...
  for (guint i = 0; i < priv->requests->len; i++) {
    ...
    if (SNAPD_IS_POST_CHANGE(data->request) &&
        g_strcmp0(_snapd_request_async_get_change_id(
...
static void send_cancel(SnapdClient *self, SnapdRequestAsync *request) {
  g_autoptr(SnapdPostChange) change_request = find_post_change_request(
      self, _snapd_request_async_get_change_id(request));
...
static SnapdRequestAsync *find_change_request(SnapdClient *self,
                                              const gchar *change_id) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
  g_autoptr(GMutexLocker) locker = g_mutex_locker_new(&priv->requests_mutex);
...
static gboolean read_chunk_header(const gchar *body, gsize body_length,
                                  gsize *chunk_header_length,
                                  gsize *chunk_length) {
...
static void complete_change(SnapdClient *self, const gchar *change_id,
                            GError *error) {
...
static void update_changes(SnapdClient *self, SnapdChange *change,
                           JsonNode *data) {
  SnapdRequestAsync *request =
      find_change_request(self, snapd_change_get_id(change));
  if (request == NULL)
    return;
  ...
  if (snapd_change_get_ready(change)) {
    ...
    if (g_cancellable_set_error_if_cancelled(
            _snapd_request_get_cancellable(SNAPD_REQUEST(request)), &error)) {
      complete_request(self, SNAPD_REQUEST(request), error);
      return;
    }

    if (snapd_change_get_error(change) != NULL) {
      g_set_error_literal(&error, SNAPD_ERROR, SNAPD_ERROR_FAILED,
...
static void parse_response(SnapdClient *self, SnapdRequest *request,
                           guint status_code, const gchar *content_type,
                           GBytes *body) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);

  g_clear_object(&priv->maintenance);
  g_autoptr(GError) error = NULL;
  if (!SNAPD_REQUEST_GET_CLASS(request)->parse_response(
          request, status_code, content_type, body, &priv->maintenance,
          &error)) {
    if (SNAPD_IS_GET_CHANGE(request)) {
      complete_change(
          self, _snapd_get_change_get_change_id(SNAPD_GET_CHANGE(request)),
          error);
      complete_request(self, request, error);
    } else if (SNAPD_IS_POST_CHANGE(request)) {
      complete_change(
          self, _snapd_post_change_get_change_id(SNAPD_POST_CHANGE(request)),
          error);
      complete_request(self, request, error);
    } else
      complete_request(self, request, error);
...
static gboolean parse_seq(SnapdClient *self, SnapdRequest *request,
                          const char *data, gsize data_length, GError **error) {
  ...
  return SNAPD_REQUEST_GET_CLASS(request)->parse_json_seq(
...
static gboolean cancel_idle_cb(gpointer user_data) {
  RequestData *data = user_data;

  g_autoptr(GError) error = NULL;
  g_cancellable_set_error_if_cancelled(
      _snapd_request_get_cancellable(data->request), &error);
  complete_request(data->client, data->request, error);

  return G_SOURCE_REMOVE;
}

static void request_cancelled_cb(GCancellable *cancellable, RequestData *data) {
  ...
  if (SNAPD_IS_REQUEST_ASYNC(data->request)) {
    ...
    g_source_set_callback(idle_source, cancel_idle_cb, request_data_ref(data),
...
static void send_request(SnapdClient *self, SnapdRequest *request) {
  ...
  if (cancellable != NULL)
    data->cancelled_id = g_cancellable_connect(
        cancellable, G_CALLBACK(request_cancelled_cb),
        request_data_new(self, request), (GDestroyNotify)request_data_unref);
  ...
  soup_message_headers_append(request_headers, "Accept-Language",
                              accept_languages);

  if (priv->auth_data != NULL) {
    g_autoptr(GString) authorization = g_string_new("");
    g_string_append_printf(authorization, "Macaroon root=\"%s\"",
    ...
    if (discharges != NULL)
      for (gsize i = 0; discharges[i] != NULL; i++)
        g_string_append_printf(authorization, ",discharge=\"%s\"",
                               discharges[i]);
    soup_message_headers_append(request_headers, "Authorization",
                                authorization->str);
  }
  if (body != NULL)
    soup_message_headers_set_content_length(request_headers,
...
static FollowLogsData *follow_logs_data_new(SnapdClient *client,
                                            SnapdLogCallback callback,
                                            gpointer callback_data) {
...
static void follow_logs_data_free(FollowLogsData *data) {
  g_slice_free(FollowLogsData, data);
}

static void log_cb(SnapdGetLogs *request, SnapdLog *log, gpointer user_data) {
 ...
 * Deprecated: 1.24
 */
void snapd_client_connect_async(SnapdClient *self, GCancellable *cancellable,
 ...
 * Since: 1.3
 */
gboolean snapd_client_connect_finish(SnapdClient *self, GAsyncResult *result,
                                     GError **error) {
 ...
 * Since: 1.24
 */
void snapd_client_set_socket_path(SnapdClient *self, const gchar *socket_path) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
 ...
 * Since: 1.16
 */
void snapd_client_set_user_agent(SnapdClient *self, const gchar *user_agent) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
 ...
 * Since: 1.16
 */
const gchar *snapd_client_get_user_agent(SnapdClient *self) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.19
 */
void snapd_client_set_allow_interaction(SnapdClient *self,
                                        gboolean allow_interaction) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
  g_return_if_fail(SNAPD_IS_CLIENT(self));
 ...
 * Since: 1.45
 */
SnapdMaintenance *snapd_client_get_maintenance(SnapdClient *self) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.19
 */
gboolean snapd_client_get_allow_interaction(SnapdClient *self) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 */
void snapd_client_login_async(SnapdClient *self, const gchar *email,
                              const gchar *password, const gchar *otp,
                              GCancellable *cancellable,
                              GAsyncReadyCallback callback,
                              gpointer user_data) {
  snapd_client_login2_async(self, email, password, otp, cancellable, callback,
                            user_data);
}
 ...
 */
SnapdAuthData *snapd_client_login_finish(SnapdClient *self,
                                         GAsyncResult *result, GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);

  g_autoptr(SnapdUserInformation) user_information =
      snapd_client_login2_finish(self, result, error);
 ...
 * Since: 1.26
 */
void snapd_client_login2_async(SnapdClient *self, const gchar *email,
                               const gchar *password, const gchar *otp,
                               GCancellable *cancellable,
                               GAsyncReadyCallback callback,
                               gpointer user_data) {
  ...
  g_autoptr(SnapdPostLogin) request = _snapd_post_login_new(
      email, password, otp, cancellable, callback, user_data);
 ...
 * Since: 1.26
 */
SnapdUserInformation *snapd_client_login2_finish(SnapdClient *self,
                                                 GAsyncResult *result,
                                                 GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.55
 */
void snapd_client_logout_async(SnapdClient *self, gint64 id,
                               GCancellable *cancellable,
                               GAsyncReadyCallback callback,
                               gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdPostLogout) request =
      _snapd_post_logout_new(id, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.55
 */
gboolean snapd_client_logout_finish(SnapdClient *self, GAsyncResult *result,
                                    GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * Since: 1.0
 */
void snapd_client_set_auth_data(SnapdClient *self, SnapdAuthData *auth_data) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
  g_return_if_fail(SNAPD_IS_CLIENT(self));
 ...
 * Since: 1.0
 */
SnapdAuthData *snapd_client_get_auth_data(SnapdClient *self) {
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * @client: a #SnapdClient.
 * @since_date_time: send only the notices generated after this moment (NULL for
 * all).
 * @timeout: time, in microseconds, to wait for a new notice (zero to return
 * immediately).
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 ...
 * Since: 1.65
 */
void snapd_client_get_notices_async(SnapdClient *self,
  ...
  snapd_client_get_notices_with_filters_async(self, NULL, NULL, NULL, NULL,
                                              since_date_time, timeout,
 ...
 *
 * Returns: (transfer container) (element-type SnapdNotice): a #GPtrArray object
 * containing the requested notices, or NULL in case of error.
 *
 * Since: 1.65
 */
GPtrArray *snapd_client_get_notices_finish(SnapdClient *self,
                                           GAsyncResult *result,
                                           GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.65
 */
void snapd_client_get_notices_with_filters_async(
    SnapdClient *self, gchar *user_id, gchar *users, gchar *types, gchar *keys,
    GDateTime *since_date_time, GTimeSpan timeout, GCancellable *cancellable,
    GAsyncReadyCallback callback, gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);

  g_autoptr(SnapdGetNotices) request =
      _snapd_get_notices_new(user_id, users, types, keys, since_date_time,
                             priv->since_date_time_nanoseconds, timeout,
                             cancellable, callback, user_data);
 ...
 * Since: 1.65
 */
GPtrArray *snapd_client_get_notices_with_filters_finish(SnapdClient *self,
                                                        GAsyncResult *result,
                                                        GError **error) {
 ...
 * Since: 1.66
 */
void snapd_client_notices_set_after_notice(SnapdClient *self,
                                           SnapdNotice *notice) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
  ...
  priv->since_date_time_nanoseconds =
      (notice == NULL) ? -1
                       : snapd_notice_get_last_occurred_nanoseconds(notice);
}
 ...
 * Since: 1.66
 */
void snapd_client_notices_set_since_nanoseconds(SnapdClient *self,
                                                int nanoseconds) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  SnapdClientPrivate *priv = snapd_client_get_instance_private(self);
 ...
 * @snap_name: (allow-none): name of snap to filter on or %NULL for changes for
 * any snap.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 ...
 * Since: 1.29
 */
void snapd_client_get_changes_async(SnapdClient *self, SnapdChangeFilter filter,
  ...
  g_autoptr(SnapdGetChanges) request = _snapd_get_changes_new(
      select, snap_name, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 *
 * Returns: (transfer container) (element-type SnapdChange): an array of
 * #SnapdChange or %NULL on error.
 *
 * Since: 1.29
 */
GPtrArray *snapd_client_get_changes_finish(SnapdClient *self,
                                           GAsyncResult *result,
                                           GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.29
 */
void snapd_client_get_change_async(SnapdClient *self, const gchar *id,
                                   GCancellable *cancellable,
                                   GAsyncReadyCallback callback,
                                   gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  ...
  g_autoptr(SnapdGetChange) request =
      _snapd_get_change_new(id, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.29
 */
SnapdChange *snapd_client_get_change_finish(SnapdClient *self,
                                            GAsyncResult *result,
                                            GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.30
 */
void snapd_client_abort_change_async(SnapdClient *self, const gchar *id,
                                     GCancellable *cancellable,
                                     GAsyncReadyCallback callback,
                                     gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  ...
  g_autoptr(SnapdPostChange) request =
      _snapd_post_change_new(id, "abort", cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.30
 */
SnapdChange *snapd_client_abort_change_finish(SnapdClient *self,
                                              GAsyncResult *result,
                                              GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.0
 */
void snapd_client_get_system_information_async(SnapdClient *self,
                                               GCancellable *cancellable,
                                               GAsyncReadyCallback callback,
                                               gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetSystemInfo) request =
      _snapd_get_system_info_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
...
SnapdSystemInformation *snapd_client_get_system_information_finish(
    SnapdClient *self, GAsyncResult *result, GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 */
void snapd_client_list_one_async(SnapdClient *self, const gchar *name,
                                 GCancellable *cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data) {
  return snapd_client_get_snap_async(self, name, cancellable, callback,
                                     user_data);
}
 ...
 */
SnapdSnap *snapd_client_list_one_finish(SnapdClient *self, GAsyncResult *result,
                                        GError **error) {
 ...
 * Since: 1.42
 */
void snapd_client_get_snap_async(SnapdClient *self, const gchar *name,
                                 GCancellable *cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetSnap) request =
      _snapd_get_snap_new(name, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.42
 */
SnapdSnap *snapd_client_get_snap_finish(SnapdClient *self, GAsyncResult *result,
                                        GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.48
 */
void snapd_client_get_snap_conf_async(SnapdClient *self, const gchar *name,
                                      GStrv keys, GCancellable *cancellable,
                                      GAsyncReadyCallback callback,
                                      gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  g_return_if_fail(name != NULL);

  g_autoptr(SnapdGetSnapConf) request =
      _snapd_get_snap_conf_new(name, keys, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * See snapd_client_get_snap_conf_sync() for more information.
 *
 * Returns: (transfer container) (element-type utf8 GVariant): a table of
 * configuration values or %NULL on error.
 *
 * Since: 1.48
 */
GHashTable *snapd_client_get_snap_conf_finish(SnapdClient *self,
                                              GAsyncResult *result,
                                              GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.48
 */
void snapd_client_set_snap_conf_async(SnapdClient *self, const gchar *name,
  ...
  g_autoptr(SnapdPutSnapConf) request = _snapd_put_snap_conf_new(
      name, key_values, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.48
 */
gboolean snapd_client_set_snap_conf_finish(SnapdClient *self,
                                           GAsyncResult *result,
                                           GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 */
void snapd_client_get_apps_async(SnapdClient *self, SnapdGetAppsFlags flags,
                                 GCancellable *cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data) {
  return snapd_client_get_apps2_async(self, flags, NULL, cancellable, callback,
                                      user_data);
}
 ...
 * See snapd_client_get_apps_sync() for more information.
 *
 * Returns: (transfer container) (element-type SnapdApp): an array of #SnapdApp
 * or %NULL on error.
 *
 * Since: 1.25
 * Deprecated: 1.45: Use snapd_client_get_apps2_finish()
 */
GPtrArray *snapd_client_get_apps_finish(SnapdClient *self, GAsyncResult *result,
                                        GError **error) {
 ...
 * @client: a #SnapdClient.
 * @flags: a set of #SnapdGetAppsFlags to control what results are returned.
 * @snaps: (allow-none): A list of snap names to return results for. If %NULL or
 * empty then apps for all installed snaps are returned.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 * @user_data: (closure): the data to pass to callback function.
 ...
 * Since: 1.45
 */
void snapd_client_get_apps2_async(SnapdClient *self, SnapdGetAppsFlags flags,
                                  GStrv snaps, GCancellable *cancellable,
                                  GAsyncReadyCallback callback,
                                  gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetApps) request =
      _snapd_get_apps_new(snaps, cancellable, callback, user_data);
 ...
 * Since: 1.45
 */
GPtrArray *snapd_client_get_apps2_finish(SnapdClient *self,
                                         GAsyncResult *result, GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.0
 */
void snapd_client_get_icon_async(SnapdClient *self, const gchar *name,
                                 GCancellable *cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetIcon) request =
      _snapd_get_icon_new(name, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.0
 */
SnapdIcon *snapd_client_get_icon_finish(SnapdClient *self, GAsyncResult *result,
                                        GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 */
void snapd_client_list_async(SnapdClient *self, GCancellable *cancellable,
                             GAsyncReadyCallback callback, gpointer user_data) {
  return snapd_client_get_snaps_async(self, SNAPD_GET_SNAPS_FLAGS_NONE, NULL,
                                      cancellable, callback, user_data);
}
 ...
 * See snapd_client_list_sync() for more information.
 *
 * Returns: (transfer container) (element-type SnapdSnap): an array of
 * #SnapdSnap or %NULL on error.
 *
 * Since: 1.0
 * Deprecated: 1.42: Use snapd_client_get_snaps_finish()
 */
GPtrArray *snapd_client_list_finish(SnapdClient *self, GAsyncResult *result,
                                    GError **error) {
 ...
 * @names: (allow-none): A list of snap names to return results for. If %NULL or
 * empty then all installed snaps are returned.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 * @user_data: (closure): the data to pass to callback function.
 ...
 * Since: 1.42
 */
void snapd_client_get_snaps_async(SnapdClient *self, SnapdGetSnapsFlags flags,
                                  GStrv names, GCancellable *cancellable,
                                  GAsyncReadyCallback callback,
                                  gpointer user_data) {
// SNAPD_GET_SNAPS_FLAGS_INCLUDE_INACTIVE and
// SNAPD_GET_SNAPS_FLAGS_REFRESH_INHIBITED are mutually exclusive; only one can
// be set.
#define SNAPD_GET_SNAPS_FLAGS_EXCLUSIVE                                        \
  (SNAPD_GET_SNAPS_FLAGS_INCLUDE_INACTIVE |                                    \
   SNAPD_GET_SNAPS_FLAGS_REFRESH_INHIBITED)
  g_assert((flags & SNAPD_GET_SNAPS_FLAGS_EXCLUSIVE) !=
           SNAPD_GET_SNAPS_FLAGS_EXCLUSIVE);
#undef SNAPD_GET_SNAPS_FLAGS_EXCLUSIVE

  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetSnaps) request =
      _snapd_get_snaps_new(cancellable, names, callback, user_data);
 ...
 * Since: 1.42
 */
GPtrArray *snapd_client_get_snaps_finish(SnapdClient *self,
                                         GAsyncResult *result, GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.8
 */
void snapd_client_get_assertions_async(SnapdClient *self, const gchar *type,
                                       GCancellable *cancellable,
                                       GAsyncReadyCallback callback,
                                       gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  ...
  g_autoptr(SnapdGetAssertions) request =
      _snapd_get_assertions_new(type, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * See snapd_client_get_assertions_sync() for more information.
 *
 * Returns: (transfer full) (array zero-terminated=1): an array of assertions or
 * %NULL on error.
 *
 * Since: 1.8
 */
GStrv snapd_client_get_assertions_finish(SnapdClient *self,
                                         GAsyncResult *result, GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.8
 */
void snapd_client_add_assertions_async(SnapdClient *self, GStrv assertions,
                                       GCancellable *cancellable,
                                       GAsyncReadyCallback callback,
                                       gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  ...
  g_autoptr(SnapdPostAssertions) request =
      _snapd_post_assertions_new(assertions, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.8
 */
gboolean snapd_client_add_assertions_finish(SnapdClient *self,
                                            GAsyncResult *result,
                                            GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 */
void snapd_client_get_interfaces_async(SnapdClient *self,
                                       GCancellable *cancellable,
                                       GAsyncReadyCallback callback,
                                       gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetInterfacesLegacy) request =
      _snapd_get_interfaces_legacy_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * @slots: (out) (allow-none) (transfer container) (element-type SnapdSlot): the
 * location to store the array of #SnapdSlot or %NULL.
 * @error: (allow-none): #GError location to store the error occurring, or %NULL
 * to ignore.
 *
 ...
 */
gboolean snapd_client_get_interfaces_finish(SnapdClient *self,
                                            GAsyncResult *result,
                                            GPtrArray **plugs,
                                            GPtrArray **slots, GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * @client: a #SnapdClient.
 * @flags: a set of #SnapdGetInterfacesFlags to control what information is
 * returned about the interfaces.
 * @names: (allow-none) (array zero-terminated=1): a null-terminated array of
 * interface names or %NULL.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 * @user_data: (closure): the data to pass to callback function.
 ...
 * Since: 1.48
 */
void snapd_client_get_interfaces2_async(SnapdClient *self,
  ...
  g_autoptr(SnapdGetInterfaces) request =
      _snapd_get_interfaces_new(names, cancellable, callback, user_data);
 ...
 * See snapd_client_get_interfaces2_sync() for more information.
 *
 * Returns: (transfer container) (element-type SnapdInterface): an array of
 * #SnapdInterface or %NULL on error.
 *
 * Since: 1.48
 */
GPtrArray *snapd_client_get_interfaces2_finish(SnapdClient *self,
                                               GAsyncResult *result,
                                               GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 */
void snapd_client_get_connections_async(SnapdClient *self,
                                        GCancellable *cancellable,
                                        GAsyncReadyCallback callback,
                                        gpointer user_data) {
  snapd_client_get_connections2_async(self, SNAPD_GET_CONNECTIONS_FLAGS_NONE,
                                      NULL, NULL, cancellable, callback,
                                      user_data);
}
...
snapd_client_get_connections_finish(SnapdClient *self, GAsyncResult *result,
                                    ...
                                    GPtrArray **slots, GError **error) {
  return snapd_client_get_connections2_finish(self, result, established,
                                              undesired, plugs, slots, error);
}

/**
 * snapd_client_get_connections2_async:
 * @client: a #SnapdClient.
 * @flags: a set of #SnapdGetConnectionsFlags to control what results are
 * returned.
 * @snap: (allow-none): the name of the snap to get connections for or %NULL for
 * all snaps.
 ...
 * Since: 1.49
 */
void snapd_client_get_connections2_async(
    SnapdClient *self, SnapdGetConnectionsFlags flags, const gchar *snap,
    const gchar *interface, GCancellable *cancellable,
    GAsyncReadyCallback callback, gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  ...
  if ((flags & SNAPD_GET_CONNECTIONS_FLAGS_SELECT_ALL) != 0)
    select = "all";
  g_autoptr(SnapdGetConnections) request = _snapd_get_connections_new(
      snap, interface, select, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
...
snapd_client_get_connections2_finish(SnapdClient *self, GAsyncResult *result,
  ...
  if (established)
    *established =
        g_ptr_array_ref(_snapd_get_connections_get_established(request));
  if (undesired)
...
void snapd_client_connect_interface_async(
    SnapdClient *self, const gchar *plug_snap, const gchar *plug_name,
  ...
  g_autoptr(SnapdPostInterfaces) request = _snapd_post_interfaces_new(
      "connect", plug_snap, plug_name, slot_snap, slot_name, progress_callback,
      progress_callback_data, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.0
 */
gboolean snapd_client_connect_interface_finish(SnapdClient *self,
                                               GAsyncResult *result,
                                               GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
...
void snapd_client_disconnect_interface_async(
  ...
  g_autoptr(SnapdPostInterfaces) request = _snapd_post_interfaces_new(
      "disconnect", plug_snap, plug_name, slot_snap, slot_name,
      progress_callback, progress_callback_data, cancellable, callback,
      user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.0
 */
gboolean snapd_client_disconnect_interface_finish(SnapdClient *self,
                                                  GAsyncResult *result,
                                                  GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_INTERFACES(result), FALSE);
 ...
 * Since: 1.0
 */
void snapd_client_find_async(SnapdClient *self, SnapdFindFlags flags,
                             const gchar *query, GCancellable *cancellable,
                             GAsyncReadyCallback callback, gpointer user_data) {
  g_return_if_fail(query != NULL);
  snapd_client_find_category_async(self, flags, NULL, query, cancellable,
                                   callback, user_data);
}
 ...
 * @client: a #SnapdClient.
 * @result: a #GAsyncResult.
 * @suggested_currency: (out) (allow-none): location to store the ISO 4217
 * currency that is suggested to purchase with.
 * @error: (allow-none): #GError location to store the error occurring, or %NULL
 * to ignore.
 *
 ...
 * Since: 1.0
 */
GPtrArray *snapd_client_find_finish(SnapdClient *self, GAsyncResult *result,
                                    gchar **suggested_currency,
                                    GError **error) {
  return snapd_client_find_category_finish(self, result, suggested_currency,
                                           error);
}
 ...
 * @client: a #SnapdClient.
 * @flags: a set of #SnapdFindFlags to control how the find is performed.
 * @section: (allow-none): store section to search in or %NULL to search in all
 * sections.
 * @query: (allow-none): query string to send or %NULL to get all snaps from the
 * given section.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 ...
 */
void snapd_client_find_section_async(SnapdClient *self, SnapdFindFlags flags,
  ...
  g_autoptr(SnapdGetFind) request =
      _snapd_get_find_new(cancellable, callback, user_data);
 ...
 */
GPtrArray *snapd_client_find_section_finish(SnapdClient *self,
                                            GAsyncResult *result,
                                            gchar **suggested_currency,
                                            GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
  ...
  if (suggested_currency != NULL)
    *suggested_currency =
        g_strdup(_snapd_get_find_get_suggested_currency(request));
 ...
 * @client: a #SnapdClient.
 * @flags: a set of #SnapdFindFlags to control how the find is performed.
 * @category: (allow-none): store category to search in or %NULL to search in
 * all categories.
 * @query: (allow-none): query string to send or %NULL to get all snaps from the
 * given category.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 ...
 * Since: 1.64
 */
void snapd_client_find_category_async(SnapdClient *self, SnapdFindFlags flags,
                                      const gchar *category, const gchar *query,
                                      GCancellable *cancellable,
                                      GAsyncReadyCallback callback,
                                      gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
 ...
 * Since: 1.64
 */
GPtrArray *snapd_client_find_category_finish(SnapdClient *self,
                                             GAsyncResult *result,
                                             gchar **suggested_currency,
                                             GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.8
 */
void snapd_client_find_refreshable_async(SnapdClient *self,
                                         GCancellable *cancellable,
                                         GAsyncReadyCallback callback,
                                         gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
 ...
 * Since: 1.5
 */
GPtrArray *snapd_client_find_refreshable_finish(SnapdClient *self,
                                                GAsyncResult *result,
                                                GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
  g_return_val_if_fail(SNAPD_IS_GET_FIND(result), NULL);
 ...
 */
void snapd_client_install_async(SnapdClient *self, const gchar *name,
                                const gchar *channel,
                                ...
                                GAsyncReadyCallback callback,
                                gpointer user_data) {
  snapd_client_install2_async(self, SNAPD_INSTALL_FLAGS_NONE, name, channel,
                              NULL, progress_callback, progress_callback_data,
                              cancellable, callback, user_data);
}
 ...
 */
gboolean snapd_client_install_finish(SnapdClient *self, GAsyncResult *result,
                                     GError **error) {
 ...
 * Since: 1.12
 */
void snapd_client_install2_async(SnapdClient *self, SnapdInstallFlags flags,
  ...
  g_autoptr(SnapdPostSnap) request = _snapd_post_snap_new(
      name, "install", progress_callback, progress_callback_data, cancellable,
      callback, user_data);
 ...
 * Since: 1.12
 */
gboolean snapd_client_install2_finish(SnapdClient *self, GAsyncResult *result,
                                      GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
...
static InstallStreamData *install_stream_data_new(SnapdClient *client,
                                                  SnapdPostSnapStream *request,
                                                  GCancellable *cancellable,
                                                  GInputStream *stream) {
...
static void install_stream_data_free(InstallStreamData *data) {
...
static void stream_read_cb(GObject *source_object, GAsyncResult *result,
                           gpointer user_data) {
  g_autoptr(InstallStreamData) data = user_data;

  g_autoptr(GError) error = NULL;
  g_autoptr(GBytes) read_data =
      g_input_stream_read_bytes_finish(data->stream, result, &error);
  ...
  if (g_bytes_get_size(read_data) == 0)
    ...
    _snapd_post_snap_stream_append_data(data->request,
                                        g_bytes_get_data(read_data, NULL),
                                        g_bytes_get_size(read_data));
    InstallStreamData *d = g_steal_pointer(&data);
    g_input_stream_read_bytes_async(d->stream, 65535, G_PRIORITY_DEFAULT,
                                    d->cancellable, stream_read_cb, d);
  }
}
...
void snapd_client_install_stream_async(
    SnapdClient *self, SnapdInstallFlags flags, GInputStream *stream,
    SnapdProgressCallback progress_callback, gpointer progress_callback_data,
    GCancellable *cancellable, GAsyncReadyCallback callback,
    gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  ...
  g_autoptr(SnapdPostSnapStream) request =
      _snapd_post_snap_stream_new(progress_callback, progress_callback_data,
                                  cancellable, callback, user_data);
  if ((flags & SNAPD_INSTALL_FLAGS_CLASSIC) != 0)
  ...
  g_input_stream_read_bytes_async(
      stream, 65535, G_PRIORITY_DEFAULT, cancellable, stream_read_cb,
      install_stream_data_new(self, request, cancellable, stream));
}
 ...
 * Since: 1.9
 */
gboolean snapd_client_install_stream_finish(SnapdClient *self,
                                            GAsyncResult *result,
                                            GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * Since: 1.9
 */
void snapd_client_try_async(SnapdClient *self, const gchar *path,
                            SnapdProgressCallback progress_callback,
                            gpointer progress_callback_data,
                            GCancellable *cancellable,
                            GAsyncReadyCallback callback, gpointer user_data) {
  ...
  g_autoptr(SnapdPostSnapTry) request =
      _snapd_post_snap_try_new(path, progress_callback, progress_callback_data,
                               cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.9
 */
gboolean snapd_client_try_finish(SnapdClient *self, GAsyncResult *result,
                                 GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * Since: 1.0
 */
void snapd_client_refresh_async(SnapdClient *self, const gchar *name,
                                const gchar *channel,
                                SnapdProgressCallback progress_callback,
                                gpointer progress_callback_data,
                                GCancellable *cancellable,
  ...
  g_autoptr(SnapdPostSnap) request = _snapd_post_snap_new(
      name, "refresh", progress_callback, progress_callback_data, cancellable,
      callback, user_data);
  _snapd_post_snap_set_channel(request, channel);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.0
 */
gboolean snapd_client_refresh_finish(SnapdClient *self, GAsyncResult *result,
                                     GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_SNAP(result), FALSE);
 ...
 * Since: 1.5
 */
void snapd_client_refresh_all_async(SnapdClient *self,
                                    SnapdProgressCallback progress_callback,
                                    gpointer progress_callback_data,
                                    GCancellable *cancellable,
                                    GAsyncReadyCallback callback,
  ...
  g_autoptr(SnapdPostSnaps) request = _snapd_post_snaps_new(
      "refresh", progress_callback, progress_callback_data, cancellable,
      callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * See snapd_client_refresh_all_sync() for more information.
 *
 * Returns: (transfer full): a %NULL-terminated array of the snap names
 * refreshed or %NULL on error.
 *
 * Since: 1.5
 */
GStrv snapd_client_refresh_all_finish(SnapdClient *self, GAsyncResult *result,
                                      GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 */
void snapd_client_remove_async(SnapdClient *self, const gchar *name,
                               SnapdProgressCallback progress_callback,
                               gpointer progress_callback_data,
                               GCancellable *cancellable,
                               GAsyncReadyCallback callback,
                               gpointer user_data) {
  snapd_client_remove2_async(self, SNAPD_REMOVE_FLAGS_NONE, name,
 ...
 */
gboolean snapd_client_remove_finish(SnapdClient *self, GAsyncResult *result,
                                    GError **error) {
 ...
 * Since: 1.50
 */
void snapd_client_remove2_async(SnapdClient *self, SnapdRemoveFlags flags,
                                const gchar *name,
                                SnapdProgressCallback progress_callback,
                                gpointer progress_callback_data,
                                GCancellable *cancellable,
  ...
  g_autoptr(SnapdPostSnap) request = _snapd_post_snap_new(
      name, "remove", progress_callback, progress_callback_data, cancellable,
      callback, user_data);
 ...
 * Since: 1.50
 */
gboolean snapd_client_remove2_finish(SnapdClient *self, GAsyncResult *result,
                                     GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_SNAP(result), FALSE);
 ...
 * Since: 1.0
 */
void snapd_client_enable_async(SnapdClient *self, const gchar *name,
                               SnapdProgressCallback progress_callback,
                               gpointer progress_callback_data,
                               GCancellable *cancellable,
                               GAsyncReadyCallback callback,
  ...
  g_autoptr(SnapdPostSnap) request = _snapd_post_snap_new(
      name, "enable", progress_callback, progress_callback_data, cancellable,
      callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.0
 */
gboolean snapd_client_enable_finish(SnapdClient *self, GAsyncResult *result,
                                    GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_SNAP(result), FALSE);
 ...
 * Since: 1.0
 */
void snapd_client_disable_async(SnapdClient *self, const gchar *name,
                                SnapdProgressCallback progress_callback,
                                gpointer progress_callback_data,
                                GCancellable *cancellable,
                                GAsyncReadyCallback callback,
  ...
  g_autoptr(SnapdPostSnap) request = _snapd_post_snap_new(
      name, "disable", progress_callback, progress_callback_data, cancellable,
      callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.0
 */
gboolean snapd_client_disable_finish(SnapdClient *self, GAsyncResult *result,
                                     GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_SNAP(result), FALSE);
 ...
 * Since: 1.26
 */
void snapd_client_switch_async(SnapdClient *self, const gchar *name,
                               const gchar *channel,
                               SnapdProgressCallback progress_callback,
                               gpointer progress_callback_data,
                               GCancellable *cancellable,
  ...
  g_autoptr(SnapdPostSnap) request = _snapd_post_snap_new(
      name, "switch", progress_callback, progress_callback_data, cancellable,
      callback, user_data);
  _snapd_post_snap_set_channel(request, channel);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.26
 */
gboolean snapd_client_switch_finish(SnapdClient *self, GAsyncResult *result,
                                    GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_SNAP(result), FALSE);
 ...
 * Since: 1.3
 */
void snapd_client_check_buy_async(SnapdClient *self, GCancellable *cancellable,
                                  GAsyncReadyCallback callback,
                                  gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetBuyReady) request =
      _snapd_get_buy_ready_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.3
 */
gboolean snapd_client_check_buy_finish(SnapdClient *self, GAsyncResult *result,
                                       GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * Since: 1.3
 */
void snapd_client_buy_async(SnapdClient *self, const gchar *id, gdouble amount,
                            const gchar *currency, GCancellable *cancellable,
                            GAsyncReadyCallback callback, gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  g_return_if_fail(id != NULL);
  g_return_if_fail(currency != NULL);

  g_autoptr(SnapdPostBuy) request = _snapd_post_buy_new(
      id, amount, currency, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.3
 */
gboolean snapd_client_buy_finish(SnapdClient *self, GAsyncResult *result,
                                 GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * @flags: a set of #SnapdCreateUserFlags to control how the user account is
 * created.
 * @cancellable: (allow-none): a #GCancellable or %NULL.
 * @callback: (scope async): a #GAsyncReadyCallback to call when the request is
 * satisfied.
 ...
 * Since: 1.3
 */
void snapd_client_create_user_async(SnapdClient *self, const gchar *email,
  ...
  g_autoptr(SnapdPostCreateUser) request =
      _snapd_post_create_user_new(email, cancellable, callback, user_data);
 ...
 * Since: 1.3
 */
SnapdUserInformation *snapd_client_create_user_finish(SnapdClient *self,
                                                      GAsyncResult *result,
                                                      GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.3
 */
void snapd_client_create_users_async(SnapdClient *self,
                                     GCancellable *cancellable,
                                     GAsyncReadyCallback callback,
                                     gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdPostCreateUsers) request =
      _snapd_post_create_users_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 *
 * Returns: (transfer container) (element-type SnapdUserInformation): an array
 * of #SnapdUserInformation or %NULL on error.
 *
 * Since: 1.3
 */
GPtrArray *snapd_client_create_users_finish(SnapdClient *self,
                                            GAsyncResult *result,
                                            GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
  ...
  if (!_snapd_request_propagate_error(SNAPD_REQUEST(request), error))
    return NULL;
  return g_ptr_array_ref(
      _snapd_post_create_users_get_users_information(request));
}
 ...
 * Since: 1.26
 */
void snapd_client_get_users_async(SnapdClient *self, GCancellable *cancellable,
                                  GAsyncReadyCallback callback,
                                  gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetUsers) request =
      _snapd_get_users_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.26
 */
GPtrArray *snapd_client_get_users_finish(SnapdClient *self,
                                         GAsyncResult *result, GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 */
void snapd_client_get_sections_async(SnapdClient *self,
                                     GCancellable *cancellable,
                                     GAsyncReadyCallback callback,
                                     gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetSections) request =
      _snapd_get_sections_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * See snapd_client_get_sections_sync() for more information.
 *
 * Returns: (transfer full) (array zero-terminated=1): an array of section names
 * or %NULL on error.
 *
 * Since: 1.7
 * Deprecated: 1.64: Use snapd_client_get_categories_finish()
 */
GStrv snapd_client_get_sections_finish(SnapdClient *self, GAsyncResult *result,
                                       GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.64
 */
void snapd_client_get_categories_async(SnapdClient *self,
                                       GCancellable *cancellable,
                                       GAsyncReadyCallback callback,
                                       gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetCategories) request =
      _snapd_get_categories_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * See snapd_client_get_categories_sync() for more information.
 *
 * Returns: (transfer container) (element-type SnapdCategoryDetails): an array
 * of #SnapdCategoryDetails or %NULL on error.
 *
 * Since: 1.64
 */
GPtrArray *snapd_client_get_categories_finish(SnapdClient *self,
                                              GAsyncResult *result,
                                              GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.8
 */
void snapd_client_get_aliases_async(SnapdClient *self,
                                    GCancellable *cancellable,
                                    GAsyncReadyCallback callback,
                                    gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetAliases) request =
      _snapd_get_aliases_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * See snapd_client_get_aliases_sync() for more information.
 *
 * Returns: (transfer container) (element-type SnapdAlias): an array of
 * #SnapdAlias or %NULL on error.
 *
 * Since: 1.8
 */
GPtrArray *snapd_client_get_aliases_finish(SnapdClient *self,
                                           GAsyncResult *result,
                                           GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
...
static void send_change_aliases_request(
    SnapdClient *self, const gchar *action, const gchar *snap, const gchar *app,
    const gchar *alias, SnapdProgressCallback progress_callback,
    gpointer progress_callback_data, GCancellable *cancellable,
    GAsyncReadyCallback callback, gpointer user_data) {
  g_autoptr(SnapdPostAliases) request = _snapd_post_aliases_new(
      action, snap, app, alias, progress_callback, progress_callback_data,
      cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.25
 */
void snapd_client_alias_async(SnapdClient *self, const gchar *snap,
                              const gchar *app, const gchar *alias,
                              SnapdProgressCallback progress_callback,
                              gpointer progress_callback_data,
                              GCancellable *cancellable,
                              GAsyncReadyCallback callback,
  ...
  send_change_aliases_request(self, "alias", snap, app, alias,
                              progress_callback, progress_callback_data,
                              cancellable, callback, user_data);
}
 ...
 * Since: 1.25
 */
gboolean snapd_client_alias_finish(SnapdClient *self, GAsyncResult *result,
                                   GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * Since: 1.25
 */
void snapd_client_unalias_async(SnapdClient *self, const gchar *snap,
                                const gchar *alias,
                                SnapdProgressCallback progress_callback,
                                gpointer progress_callback_data,
                                GCancellable *cancellable,
  ...
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  g_return_if_fail(alias != NULL);
  send_change_aliases_request(self, "unalias", snap, NULL, alias,
                              progress_callback, progress_callback_data,
                              cancellable, callback, user_data);
}
 ...
 * Since: 1.25
 */
gboolean snapd_client_unalias_finish(SnapdClient *self, GAsyncResult *result,
                                     GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_ALIASES(result), FALSE);
 ...
 * Since: 1.25
 */
void snapd_client_prefer_async(SnapdClient *self, const gchar *snap,
                               SnapdProgressCallback progress_callback,
                               gpointer progress_callback_data,
                               GCancellable *cancellable,
                               GAsyncReadyCallback callback,
  ...
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  g_return_if_fail(snap != NULL);
  send_change_aliases_request(self, "prefer", snap, NULL, NULL,
                              progress_callback, progress_callback_data,
                              cancellable, callback, user_data);
}
 ...
 * Since: 1.25
 */
gboolean snapd_client_prefer_finish(SnapdClient *self, GAsyncResult *result,
                                    GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_ALIASES(result), FALSE);
 ...
 */
void snapd_client_enable_aliases_async(SnapdClient *self, const gchar *snap,
                                       GStrv aliases,
                                       SnapdProgressCallback progress_callback,
                                       gpointer progress_callback_data,
                                       GCancellable *cancellable,
  ...
  g_autoptr(GTask) task = g_task_new(self, cancellable, callback, user_data);
  g_task_return_new_error(task, SNAPD_ERROR, SNAPD_ERROR_FAILED,
                          "snapd_client_enable_aliases_async is deprecated");
}
 ...
 */
gboolean snapd_client_enable_aliases_finish(SnapdClient *self,
                                            GAsyncResult *result,
                                            GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(g_task_is_valid(result, self), FALSE);
 ...
 */
void snapd_client_disable_aliases_async(SnapdClient *self, const gchar *snap,
                                        GStrv aliases,
                                        SnapdProgressCallback progress_callback,
                                        gpointer progress_callback_data,
                                        GCancellable *cancellable,
  ...
  g_autoptr(GTask) task = g_task_new(self, cancellable, callback, user_data);
  g_task_return_new_error(task, SNAPD_ERROR, SNAPD_ERROR_FAILED,
                          "snapd_client_disable_aliases_async is deprecated");
}
 ...
 * Deprecated: 1.25: Use snapd_client_unalias_finish()
 */
gboolean snapd_client_disable_aliases_finish(SnapdClient *self,
                                             GAsyncResult *result,
                                             GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(g_task_is_valid(result, self), FALSE);
 ...
 */
void snapd_client_reset_aliases_async(SnapdClient *self, const gchar *snap,
                                      GStrv aliases,
                                      SnapdProgressCallback progress_callback,
                                      gpointer progress_callback_data,
                                      GCancellable *cancellable,
  ...
  g_autoptr(GTask) task = g_task_new(self, cancellable, callback, user_data);
  g_task_return_new_error(task, SNAPD_ERROR, SNAPD_ERROR_FAILED,
                          "snapd_client_reset_aliases_async is deprecated");
}
 ...
 */
gboolean snapd_client_reset_aliases_finish(SnapdClient *self,
                                           GAsyncResult *result,
                                           GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(g_task_is_valid(result, self), FALSE);
 ...
 */
void snapd_client_run_snapctl_async(SnapdClient *self, const gchar *context_id,
                                    GStrv args, GCancellable *cancellable,
                                    GAsyncReadyCallback callback,
                                    gpointer user_data) {
  return snapd_client_run_snapctl2_async(self, context_id, args, cancellable,
                                         callback, user_data);
}
 ...
 * @client: a #SnapdClient.
 * @result: a #GAsyncResult.
 * @stdout_output: (out) (allow-none): the location to write the stdout from the
 * command or %NULL.
 * @stderr_output: (out) (allow-none): the location to write the stderr from the
 * command or %NULL.
 * @error: (allow-none): #GError location to store the error occurring, or %NULL
 * to ignore.
 *
 ...
 */
gboolean snapd_client_run_snapctl_finish(SnapdClient *self,
                                         GAsyncResult *result,
                                         gchar **stdout_output,
                                         gchar **stderr_output,
                                         GError **error) {
  ...
  return snapd_client_run_snapctl2_finish(self, result, stdout_output,
                                          stderr_output, NULL, error);
}
 ...
 * Since: 1.59
 */
void snapd_client_run_snapctl2_async(SnapdClient *self, const gchar *context_id,
                                     GStrv args, GCancellable *cancellable,
                                     GAsyncReadyCallback callback,
                                     gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  ...
  g_autoptr(SnapdPostSnapctl) request = _snapd_post_snapctl_new(
      context_id, args, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * @stderr_output: (out) (allow-none): the location to write the stderr from the
 * command or %NULL.
 * @exit_code: (out) (allow-none): the location to write the exit code of the
 * command or %NULL.
 * @error: (allow-none): #GError location to store the error occurring, or %NULL
 * to ignore.
 *
 ...
 * Since: 1.59
 */
gboolean snapd_client_run_snapctl2_finish(SnapdClient *self,
                                          GAsyncResult *result,
                                          gchar **stdout_output,
                                          gchar **stderr_output, int *exit_code,
                                          GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
  g_return_val_if_fail(SNAPD_IS_POST_SNAPCTL(result), FALSE);
 ...
 * Since: 1.54
 */
void snapd_client_download_async(SnapdClient *self, const gchar *name,
                                 const gchar *channel, const gchar *revision,
                                 GCancellable *cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));
  ...
  g_autoptr(SnapdPostDownload) request = _snapd_post_download_new(
      name, channel, revision, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.54
 */
GBytes *snapd_client_download_finish(SnapdClient *self, GAsyncResult *result,
                                     GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.60
 */
void snapd_client_check_themes_async(SnapdClient *self, GStrv gtk_theme_names,
  ...
  g_autoptr(SnapdGetThemes) request = _snapd_get_themes_new(
      gtk_theme_names, icon_theme_names, sound_theme_names, cancellable,
      callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.60
 */
gboolean snapd_client_check_themes_finish(SnapdClient *self,
                                          GAsyncResult *result,
  ...
  if (gtk_theme_status)
    *gtk_theme_status =
        g_hash_table_ref(_snapd_get_themes_get_gtk_theme_status(request));
  if (icon_theme_status)
    *icon_theme_status =
        g_hash_table_ref(_snapd_get_themes_get_icon_theme_status(request));
  if (sound_theme_status)
    *sound_theme_status =
        g_hash_table_ref(_snapd_get_themes_get_sound_theme_status(request));
 ...
 * Since: 1.60
 */
void snapd_client_install_themes_async(
    SnapdClient *self, GStrv gtk_theme_names, GStrv icon_theme_names,
    GStrv sound_theme_names, SnapdProgressCallback progress_callback,
    gpointer progress_callback_data, GCancellable *cancellable,
    GAsyncReadyCallback callback, gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdPostThemes) request = _snapd_post_themes_new(
      gtk_theme_names, icon_theme_names, sound_theme_names, progress_callback,
      progress_callback_data, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.60
 */
gboolean snapd_client_install_themes_finish(SnapdClient *self,
                                            GAsyncResult *result,
                                            GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * Since: 1.64
 */
void snapd_client_get_logs_async(SnapdClient *self, GStrv names, size_t n,
                                 GCancellable *cancellable,
                                 GAsyncReadyCallback callback,
                                 gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetLogs) request = _snapd_get_logs_new(
      names, n, FALSE, NULL, NULL, NULL, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * See snapd_client_get_logs_sync() for more information.
 *
 * Returns: (transfer container) (element-type SnapdLog): an array of #SnapdLog
 * or %NULL on error.
 *
 * Since: 1.64
 */
GPtrArray *snapd_client_get_logs_finish(SnapdClient *self, GAsyncResult *result,
                                        GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.64
 */
void snapd_client_follow_logs_async(SnapdClient *self, GStrv names,
  ...
  g_autoptr(SnapdGetLogs) request = _snapd_get_logs_new(
      names, 0, TRUE, log_cb,
      follow_logs_data_new(self, log_callback, log_callback_data),
      (GDestroyNotify)follow_logs_data_free, cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.64
 */
gboolean snapd_client_follow_logs_finish(SnapdClient *self,
                                         GAsyncResult *result, GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), FALSE);
 ...
 * Since: 1.67
 */
void snapd_client_get_model_assertion_async(SnapdClient *self,
                                            GCancellable *cancellable,
                                            GAsyncReadyCallback callback,
                                            gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetModel) request =
      _snapd_get_model_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.67
 */
gchar *snapd_client_get_model_assertion_finish(SnapdClient *self,
                                               GAsyncResult *result,
                                               GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
 ...
 * Since: 1.67
 */
void snapd_client_get_serial_assertion_async(SnapdClient *self,
                                             GCancellable *cancellable,
                                             GAsyncReadyCallback callback,
                                             gpointer user_data) {
  g_return_if_fail(SNAPD_IS_CLIENT(self));

  g_autoptr(SnapdGetModelSerial) request =
      _snapd_get_model_serial_new(cancellable, callback, user_data);
  send_request(self, SNAPD_REQUEST(request));
}
 ...
 * Since: 1.67
 */
gchar *snapd_client_get_serial_assertion_finish(SnapdClient *self,
                                                GAsyncResult *result,
                                                GError **error) {
  g_return_val_if_fail(SNAPD_IS_CLIENT(self), NULL);
...
SnapdClient *snapd_client_new_from_socket(GSocket *socket) {
  ...
  SnapdClientPrivate *priv =
      snapd_client_get_instance_private(SNAPD_CLIENT(self));
...
static void snapd_client_finalize(GObject *object) {
  SnapdClientPrivate *priv =
...
static void snapd_client_class_init(SnapdClientClass *klass) {
