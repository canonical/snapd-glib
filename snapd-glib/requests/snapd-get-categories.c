...
G_DEFINE_TYPE(SnapdGetCategories, snapd_get_categories,
              snapd_request_get_type())

SnapdGetCategories *_snapd_get_categories_new(GCancellable *cancellable,
                                              GAsyncReadyCallback callback,
                                              gpointer user_data) {
  return SNAPD_GET_CATEGORIES(g_object_new(
      snapd_get_categories_get_type(), "cancellable", cancellable,
      "ready-callback", callback, "ready-callback-data", user_data, NULL));
}

GPtrArray *_snapd_get_categories_get_categories(SnapdGetCategories *self) {
  return self->categories;
}

static SoupMessage *generate_get_categories_request(SnapdRequest *request,
                                                    GBytes **body) {
  return soup_message_new("GET", "http://snapd/v2/categories");
}

static gboolean
parse_get_categories_response(SnapdRequest *request, guint status_code,
  ...
  for (guint i = 0; i < json_array_get_length(result); i++) {
    ...
    details =
        g_object_new(snapd_category_details_get_type(), "name", name, NULL);
...
static void snapd_get_categories_finalize(GObject *object) {
...
static void snapd_get_categories_class_init(SnapdGetCategoriesClass *klass) {
...
static void snapd_get_categories_init(SnapdGetCategories *self) {}
