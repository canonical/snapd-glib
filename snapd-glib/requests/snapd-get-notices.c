...
_snapd_get_notices_new(gchar *user_id, gchar *users, gchar *types, gchar *keys,
                       GDateTime *since_date_time,
                       int since_date_time_nanoseconds, GTimeSpan timeout,
                       GCancellable *cancellable, GAsyncReadyCallback callback,
                       gpointer user_data) {
  SnapdGetNotices *self = SNAPD_GET_NOTICES(g_object_new(
      snapd_get_notices_get_type(), "cancellable", cancellable,
  ...
  self->since_date_time =
      since_date_time == NULL ? NULL : g_date_time_ref(since_date_time);
  self->timeout = timeout;
  self->since_date_time_nanoseconds = since_date_time_nanoseconds;
  return self;
}

static void add_uri_parameter_base(GString *query, const gchar *name) {
  if (query->len != 0)
...
static void add_uri_parameter(GString *query, const gchar *name,
                              const gchar *value) {
...
static SoupMessage *generate_get_snap_request(SnapdRequest *request,
                                              GBytes **body) {
  ...
  if (self->since_date_time != NULL) {
    ...
    if (self->since_date_time_nanoseconds == -1) {
      ...
      if (self->since_date_time_nanoseconds == 0) {
        ...
        // create a temporal date-time string with a %09d between seconds and
        // timezone
        g_autofree gchar *date_time_tmp =
            g_date_time_format(self->since_date_time, "%FT%T.%%09d%:z");
        // Now use that %s to include the nanoseconds
        date_time =
            g_strdup_printf(date_time_tmp, self->since_date_time_nanoseconds);
      }
    }
  ...
  if (self->timeout != 0) {
    ...
    // timeout in microseconds. self->timeout is a GTimeSpan, which is a typedef
    // into a gint64 type
...
static gboolean parse_get_snap_response(SnapdRequest *request,
                                        guint status_code,
...
static void snapd_get_notices_finalize(GObject *object) {
...
GPtrArray *_snapd_get_notices_get_notices(SnapdGetNotices *self) {
  g_return_val_if_fail(SNAPD_IS_GET_NOTICES(self), NULL);
  return self->notices;
}

static void snapd_get_notices_class_init(SnapdGetNoticesClass *klass) {
...
static void snapd_get_notices_init(SnapdGetNotices *self) {
  self->since_date_time_nanoseconds = -1;
}
