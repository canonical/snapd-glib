...
SnapdPostSnaps *_snapd_post_snaps_new(const gchar *action,
                                      ...
                                      gpointer user_data) {
  SnapdPostSnaps *self = SNAPD_POST_SNAPS(
      g_object_new(snapd_post_snaps_get_type(), "cancellable", cancellable,
...
GStrv _snapd_post_snaps_get_snap_names(SnapdPostSnaps *self) {
  return self->snap_names;
}

static SoupMessage *generate_post_snaps_request(SnapdRequest *request,
                                                GBytes **body) {
...
static gboolean parse_post_snaps_result(SnapdRequestAsync *request,
                                        JsonNode *result, GError **error) {
  SnapdPostSnaps *self = SNAPD_POST_SNAPS(request);

  if (result == NULL || json_node_get_value_type(result) != JSON_TYPE_OBJECT) {
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
...
static void snapd_post_snaps_finalize(GObject *object) {
...
static void snapd_post_snaps_class_init(SnapdPostSnapsClass *klass) {
  SnapdRequestClass *request_class = SNAPD_REQUEST_CLASS(klass);
  SnapdRequestAsyncClass *request_async_class =
      SNAPD_REQUEST_ASYNC_CLASS(klass);
...
static void snapd_post_snaps_init(SnapdPostSnaps *self) {}
