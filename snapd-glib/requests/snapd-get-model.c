...
SnapdGetModel *
_snapd_get_model_new (GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    return SNAPD_GET_MODEL (g_object_new (snapd_get_model_get_type (),
...
const gchar *
_snapd_get_model_get_model_assertion (SnapdGetModel *self)
{
    return self->model_assertion;
}

static SoupMessage *
generate_get_model_request (SnapdRequest *request, GBytes **body)
{
    return soup_message_new ("GET", "http://snapd/v2/model");
}

static gboolean
parse_get_model_response (SnapdRequest *request, guint status_code, const gchar *content_type, GBytes *body, SnapdMaintenance **maintenance, GError **error)
{
    ...
    if (g_strcmp0 (content_type, "application/x.ubuntu.assertion") != 0) {
        g_set_error (error,
                     ...
                     "Got unknown content type '%s' retrieving model assertion", content_type);
        return FALSE;
    }
...
static void
snapd_get_model_finalize (GObject *object)
{
...
static void
snapd_get_model_class_init (SnapdGetModelClass *klass)
{
...
static void
snapd_get_model_init (SnapdGetModel *self)
{
}
