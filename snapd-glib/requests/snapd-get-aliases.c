...
SnapdGetAliases *_snapd_get_aliases_new(GCancellable *cancellable,
                                        ...
                                        gpointer user_data) {
  return SNAPD_GET_ALIASES(g_object_new(
      snapd_get_aliases_get_type(), "cancellable", cancellable,
      "ready-callback", callback, "ready-callback-data", user_data, NULL));
}

GPtrArray *_snapd_get_aliases_get_aliases(SnapdGetAliases *self) {
  return self->aliases;
}

static SoupMessage *generate_get_aliases_request(SnapdRequest *request,
                                                 GBytes **body) {
  return soup_message_new("GET", "http://snapd/v2/aliases");
}

static gboolean
parse_get_aliases_response(SnapdRequest *request, guint status_code,
  ...
  while (json_object_iter_next(&snap_iter, &snap, &snap_node)) {
    ...
    while (json_object_iter_next(&alias_iter, &name, &alias_node)) {
      SnapdAlias *alias =
          _snapd_json_parse_alias(alias_node, snap, name, error);
      if (alias == NULL)
        return FALSE;
...
static void snapd_get_aliases_finalize(GObject *object) {
...
static void snapd_get_aliases_class_init(SnapdGetAliasesClass *klass) {
...
static void snapd_get_aliases_init(SnapdGetAliases *self) {}
