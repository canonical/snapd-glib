...
void _snapd_json_set_body(SoupMessage *message, JsonBuilder *builder,
                          GBytes **body) {
...
#endif
  soup_message_headers_set_content_type(request_headers, "application/json",
                                        NULL);
  ...
  gsize data_length;
  g_autofree guchar *data =
      (guchar *)json_generator_to_data(json_generator, &data_length);
  *body = g_bytes_new_take(g_steal_pointer(&data), data_length);
}

gboolean _snapd_json_get_bool(JsonObject *object, const gchar *name,
...
gint64 _snapd_json_get_int(JsonObject *object, const gchar *name,
...
const gchar *_snapd_json_get_string(JsonObject *object, const gchar *name,
...
GList *_snapd_json_get_keys(JsonObject *object) {
  GList *keys = NULL;
...
JsonObject *_snapd_json_get_object(JsonObject *object, const gchar *name) {
  JsonNode *node = json_object_get_member(object, name);
...
gboolean _snapd_json_parse_time_span(const gchar *time_span_string,
                                     GTimeSpan *time_span) {
  /* Examples:
        3y
        1y5d
...
static gboolean parse_date(const gchar *date_string, gint *year, gint *month,
                           gint *day) {
  /* Example: 2016-05-17 */
...
static gboolean parse_time(const gchar *time_string, gint *hour, gint *minute,
                           gdouble *seconds) {
...
static gboolean is_timezone_prefix(gchar c) {
  return c == '+' || c == '-' || c == 'Z';
}

static int parse_nanoseconds(gchar *token) {
  gchar *nanostr;
  int nanosecs;
  size_t nanostr_len;

  // find the decimal dot
  for (nanostr = token; (*nanostr != '.') && (*nanostr != 0); nanostr++)
    ;
  ...
  nanostr++; // jump over the dot
  nanosecs = atoi(nanostr);

  // RFC3999 removes trailing zeros, so we must complete up to nine decimals
  nanostr_len = strlen(nanostr);
  if (nanostr_len < 9)
    for (int i = 0; i < (9 - nanostr_len); i++)
      nanosecs *= 10;
  return nanosecs;
}

GDateTime *_snapd_json_get_date_time(JsonObject *object, const gchar *name,
                                     int *nanoseconds) {
  ...
  if (nanoseconds != NULL)
    *nanoseconds = parse_nanoseconds(tokens[1]);
  return g_date_time_new(timezone, year, month, day, hour, minute, seconds);
}

static void parse_error_response(JsonObject *root, JsonNode **error_value,
                                 GError **error) {
  JsonObject *result = _snapd_json_get_object(root, "result");
  gint64 status_code = _snapd_json_get_int(root, "status-code", 0);
  const gchar *kind =
      result != NULL ? _snapd_json_get_string(result, "kind", NULL) : NULL;
  const gchar *message =
      result != NULL ? _snapd_json_get_string(result, "message", NULL) : NULL;

  if (error_value != NULL) {
    *error_value =
        result != NULL ? json_object_get_member(result, "value") : NULL;
    if (*error_value != NULL)
      json_node_ref(*error_value);
  }

  if (g_strcmp0(kind, "login-required") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_AUTH_DATA_REQUIRED,
                        message);
  else if (g_strcmp0(kind, "invalid-auth-data") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_AUTH_DATA_INVALID,
                        message);
  else if (g_strcmp0(kind, "two-factor-required") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_TWO_FACTOR_REQUIRED,
                        message);
  else if (g_strcmp0(kind, "two-factor-failed") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_TWO_FACTOR_INVALID,
                        message);
  else if (g_strcmp0(kind, "terms-not-accepted") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_TERMS_NOT_ACCEPTED,
                        message);
  else if (g_strcmp0(kind, "no-payment-methods") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_PAYMENT_NOT_SETUP,
                        message);
  else if (g_strcmp0(kind, "payment-declined") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_PAYMENT_DECLINED,
                        message);
  else if (g_strcmp0(kind, "snap-already-installed") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_ALREADY_INSTALLED,
                        message);
  else if (g_strcmp0(kind, "snap-not-installed") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NOT_INSTALLED, message);
  else if (g_strcmp0(kind, "snap-not-found") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NOT_FOUND, message);
  else if (g_strcmp0(kind, "snap-local") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NOT_IN_STORE, message);
  else if (g_strcmp0(kind, "snap-no-update-available") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NO_UPDATE_AVAILABLE,
                        message);
  else if (g_strcmp0(kind, "password-policy") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_PASSWORD_POLICY_ERROR,
                        message);
  else if (g_strcmp0(kind, "snap-needs-devmode") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NEEDS_DEVMODE, message);
  else if (g_strcmp0(kind, "snap-needs-classic") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NEEDS_CLASSIC, message);
  else if (g_strcmp0(kind, "snap-needs-classic-system") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NEEDS_CLASSIC_SYSTEM,
                        message);
  else if (g_strcmp0(kind, "bad-query") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_BAD_QUERY, message);
  else if (g_strcmp0(kind, "network-timeout") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NETWORK_TIMEOUT,
                        message);
  else if (g_strcmp0(kind, "auth-cancelled") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_AUTH_CANCELLED,
                        message);
  else if (g_strcmp0(kind, "snap-not-classic") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NOT_CLASSIC, message);
  else if (g_strcmp0(kind, "snap-revision-not-available") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_REVISION_NOT_AVAILABLE,
                        message);
  else if (g_strcmp0(kind, "snap-channel-not-available") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_CHANNEL_NOT_AVAILABLE,
                        message);
  else if (g_strcmp0(kind, "snap-not-a-snap") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_NOT_A_SNAP, message);
  else if (g_strcmp0(kind, "dns-failure") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_DNS_FAILURE, message);
  else if (g_strcmp0(kind, "option-not-found") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_OPTION_NOT_FOUND,
                        message);
  else if (g_strcmp0(kind, "unsuccessful") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_UNSUCCESSFUL, message);
  else if (g_strcmp0(kind, "app-not-found") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_APP_NOT_FOUND, message);
  else if (g_strcmp0(kind, "snap-architecture-not-available") == 0)
    g_set_error_literal(error, SNAPD_ERROR,
                        SNAPD_ERROR_ARCHITECTURE_NOT_AVAILABLE, message);
  else if (g_strcmp0(kind, "snap-change-conflict") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_CHANGE_CONFLICT,
                        message);
  else if (g_strcmp0(kind, "interfaces-unchanged") == 0)
    g_set_error_literal(error, SNAPD_ERROR, SNAPD_ERROR_INTERFACES_UNCHANGED,
                        message);
  else {
    switch (status_code) {
    case SOUP_STATUS_BAD_REQUEST:
      g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_BAD_REQUEST, "%s: %s", kind,
                  message);
      break;
    case SOUP_STATUS_UNAUTHORIZED:
      g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_AUTH_DATA_REQUIRED, "%s: %s",
                  kind, message);
      break;
    case SOUP_STATUS_FORBIDDEN:
      g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_PERMISSION_DENIED, "%s: %s",
                  kind, message);
      break;
    case SOUP_STATUS_NOT_FOUND:
      g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_NOT_FOUND, "%s: %s", kind,
                  message);
      break;
     ...
     */
    default:
      g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_FAILED,
                  "status-code=%" G_GINT64_FORMAT " kind=%s message=%s",
                  status_code, kind, message);
    }
  }
}

JsonObject *_snapd_json_parse_response(const gchar *content_type, GBytes *body,
                                       SnapdMaintenance **maintenance,
                                       JsonNode **error_value, GError **error) {
  if (content_type == NULL) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_BAD_RESPONSE,
  ...
  if (!json_parser_load_from_data(parser, g_bytes_get_data(body, NULL),
                                  g_bytes_get_size(body), &error_local)) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_BAD_RESPONSE,
                "Unable to parse snapd response: %s", error_local->message);
    return NULL;
  }
  ...
  if (type_node == NULL ||
    ...
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_BAD_RESPONSE,
                "snapd response does not have a type");
    return NULL;
  }
  ...
  if (json_object_has_member(root, "maintenance") && maintenance != NULL) {
    ...
    *maintenance = g_object_new(
        SNAPD_TYPE_MAINTENANCE, "kind", maintenance_kind, "message",
        _snapd_json_get_string(m, "message", NULL), NULL);
  }
...
JsonNode *_snapd_json_get_sync_result(JsonObject *response, GError **error) {
  ...
  if (strcmp(type, "sync") != 0) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                "Unexpected response '%s' returned for sync request", type);
    return NULL;
  }
...
JsonObject *_snapd_json_get_sync_result_o(JsonObject *response,
  ...
  /*g_autoptr(JsonNode) result = _snapd_json_get_sync_result (response,
   * error);*/
...
JsonArray *_snapd_json_get_sync_result_a(JsonObject *response, GError **error) {
  /* FIXME: Needs json-glib to be fixed to use json_node_unref */
  /*g_autoptr(JsonNode) result = _snapd_json_get_sync_result (response,
   * error);*/
  JsonNode *result = _snapd_json_get_sync_result(response, error);
...
gchar *_snapd_json_get_async_result(JsonObject *response, GError **error) {
  const gchar *type = json_object_get_string_member(response, "type");
  if (strcmp(type, "async") != 0) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                "Unexpected response '%s' returned for async request", type);
    return NULL;
  }

  JsonNode *change_node = json_object_get_member(response, "change");
  if (change_node == NULL ||
      json_node_get_value_type(change_node) != G_TYPE_STRING) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                "No change returned for async request");
    return NULL;
  }
...
static GStrv create_str_array_from_jsonarray(JsonArray *data) {
  if (data == NULL)
    return NULL;
...
static gchar *get_int_as_string(JsonObject *object, const gchar *name) {
  JsonNode *node = json_object_get_member(object, name);
  if (node == NULL)
    return NULL;
...
static SnapdNotice *parse_notice(JsonObject *object) {
  int last_occurred_nanoseconds = 0;
  g_autoptr(GDateTime) first_occurred =
      _snapd_json_get_date_time(object, "first-occurred", NULL);
  g_autoptr(GDateTime) last_occurred = _snapd_json_get_date_time(
      object, "last-occurred", &last_occurred_nanoseconds);
  g_autoptr(GDateTime) last_repeated =
      _snapd_json_get_date_time(object, "last-repeated", NULL);

  GTimeSpan expire_after = 0, repeat_after = 0;
  _snapd_json_parse_time_span(
      _snapd_json_get_string(object, "expire-after", NULL), &expire_after);
  _snapd_json_parse_time_span(
      _snapd_json_get_string(object, "repeat-after", NULL), &repeat_after);

  const gchar *notice_type_string =
      _snapd_json_get_string(object, "type", NULL);
  ...
  if (json_object_has_member(object, "last-data")) {
    ...
    while (json_object_iter_next(&iter, &member_name, &member_node))
      g_hash_table_insert(last_data, g_strdup(member_name),
                          g_strdup(json_node_get_string(member_node)));
  }
  g_autofree gchar *user_id = get_int_as_string(object, "user-id");
  return g_object_new(
      SNAPD_TYPE_NOTICE, "id", _snapd_json_get_string(object, "id", NULL),
      "user-id", user_id, "notice-type", notice_type, "key",
      _snapd_json_get_string(object, "key", NULL), "first-occurred",
      first_occurred, "last-occurred", last_occurred,
      "last-occurred-nanoseconds", last_occurred_nanoseconds, "last-repeated",
      last_repeated, "occurrences",
      _snapd_json_get_int(object, "occurrences", -1), "expire-after",
      expire_after, "repeat-after", repeat_after, "last-data", last_data, NULL);
}

GPtrArray *_snapd_json_parse_notices(JsonNode *node, GError **error) {
  ...
  g_autoptr(GPtrArray) notices = g_ptr_array_new_with_free_func(g_object_unref);
  for (guint i = 0; i < json_array_get_length(array); i++) {
    ...
    g_ptr_array_add(notices, parse_notice(object));
  }

  return g_steal_pointer(&notices);
}

SnapdChange *_snapd_json_parse_change(JsonNode *node, GError **error) {
  ...
  for (guint i = 0; i < json_array_get_length(array); i++) {
    ...
    g_autoptr(GDateTime) spawn_time =
        _snapd_json_get_date_time(object, "spawn-time", NULL);
    g_autoptr(GDateTime) ready_time =
        _snapd_json_get_date_time(object, "ready-time", NULL);
    JsonObject *data = _snapd_json_get_object(object, "data");
    g_autoptr(SnapdTaskData) task_data = NULL;
    if (data != NULL) {
      JsonNode *affected_snaps_node =
          json_object_get_member(data, "affected-snaps");
      if (affected_snaps_node != NULL) {
        ...
        g_auto(GStrv) affected_snaps_array =
            create_str_array_from_jsonarray(affected_snaps);
        task_data = g_object_new(SNAPD_TYPE_TASK_DATA, "affected-snaps",
                                 affected_snaps_array, NULL);
      }
    }

    g_autoptr(SnapdTask) t = g_object_new(
        SNAPD_TYPE_TASK, "id", _snapd_json_get_string(object, "id", NULL),
        "kind", _snapd_json_get_string(object, "kind", NULL), "summary",
        _snapd_json_get_string(object, "summary", NULL), "status",
        _snapd_json_get_string(object, "status", NULL), "progress-label",
        progress != NULL ? _snapd_json_get_string(progress, "label", NULL)
                         : NULL,
        "progress-done",
        progress != NULL ? _snapd_json_get_int(progress, "done", 0) : 0,
        "progress-total",
        progress != NULL ? _snapd_json_get_int(progress, "total", 0) : 0,
        "spawn-time", spawn_time, "ready-time", ready_time, "data", task_data,
        NULL);
    g_ptr_array_add(tasks, g_steal_pointer(&t));
  }

  g_autoptr(GDateTime) main_spawn_time =
      _snapd_json_get_date_time(object, "spawn-time", NULL);
  g_autoptr(GDateTime) main_ready_time =
      _snapd_json_get_date_time(object, "ready-time", NULL);
  ...
  // Currently, only one kind of Change can have DATA field, so this is quite
  // "hardcoded". When more Change kinds have DATA field, new subclasses of
  // SNAPD_TYPE_CHANGE_DATA must be created, and the code in QT's
  // snapd-change-data.c file must be completed to manage the new types.
  if (g_str_equal(kind, "auto-refresh") && (autorefresh_data != NULL)) {

    g_autoptr(JsonArray) snap_names_json =
        _snapd_json_get_array(autorefresh_data, "snap-names");
    g_auto(GStrv) snap_names = create_str_array_from_jsonarray(snap_names_json);

    g_autoptr(JsonArray) refresh_forced_json =
        _snapd_json_get_array(autorefresh_data, "refresh-forced");
    g_auto(GStrv) refresh_forced =
        create_str_array_from_jsonarray(refresh_forced_json);

    data = g_object_new(SNAPD_TYPE_AUTOREFRESH_CHANGE_DATA, "snap-names",
                        snap_names, "refresh-forced", refresh_forced, NULL);
  }

  return g_object_new(
      SNAPD_TYPE_CHANGE, "id", _snapd_json_get_string(object, "id", NULL),
      "kind", _snapd_json_get_string(object, "kind", NULL), "summary",
      _snapd_json_get_string(object, "summary", NULL), "status",
      _snapd_json_get_string(object, "status", NULL), "tasks", tasks, "ready",
      _snapd_json_get_bool(object, "ready", FALSE), "spawn-time",
      main_spawn_time, "ready-time", main_ready_time, "error",
      _snapd_json_get_string(object, "err", NULL), "data", data, NULL);
}

static SnapdConfinement parse_confinement(const gchar *value) {
...
SnapdSystemInformation *_snapd_json_parse_system_information(JsonNode *node,
  ...
  const gchar *architecture =
      _snapd_json_get_string(object, "architecture", "");
  const gchar *confinement_string =
      _snapd_json_get_string(object, "confinement", "");
  ...
  JsonObject *sandbox_features =
      _snapd_json_get_object(object, "sandbox-features");
  g_autoptr(GHashTable) sandbox_features_hash = g_hash_table_new_full(
  ...
  if (sandbox_features != NULL) {
    ...
    while (json_object_iter_next(&iter, &name, &features_node)) {
      if (json_node_get_value_type(features_node) != JSON_TYPE_ARRAY) {
        g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                    "Unexpected sandbox features type");
        return FALSE;
      }
      ...
      for (guint i = 0; i < json_array_get_length(features_array); i++) {
        ...
        g_ptr_array_add(features_ptr_array,
                        (gpointer)json_node_get_string(node));
      }
      g_ptr_array_add(features_ptr_array, NULL);
      g_hash_table_insert(sandbox_features_hash, g_strdup(name),
                          g_strdupv((GStrv)features_ptr_array->pdata));
    }
  }

  g_autoptr(GDateTime) refresh_hold =
      _snapd_json_get_date_time(refresh, "hold", NULL);
  g_autoptr(GDateTime) refresh_last =
      _snapd_json_get_date_time(refresh, "last", NULL);
  g_autoptr(GDateTime) refresh_next =
      _snapd_json_get_date_time(refresh, "next", NULL);

  return g_object_new(
      SNAPD_TYPE_SYSTEM_INFORMATION, "architecture", architecture,
      "binaries-directory",
      locations != NULL
          ? _snapd_json_get_string(locations, "snap-bin-dir", NULL)
          : NULL,
      "build-id", _snapd_json_get_string(object, "build-id", NULL),
      "confinement", confinement, "kernel-version",
      _snapd_json_get_string(object, "kernel-version", NULL), "managed",
      _snapd_json_get_bool(object, "managed", FALSE), "mount-directory",
      locations != NULL
          ? _snapd_json_get_string(locations, "snap-mount-dir", NULL)
          : NULL,
      "on-classic", _snapd_json_get_bool(object, "on-classic", FALSE), "os-id",
      os_release != NULL ? _snapd_json_get_string(os_release, "id", NULL)
                         : NULL,
      "os-version",
      os_release != NULL
          ? _snapd_json_get_string(os_release, "version-id", NULL)
          : NULL,
      "sandbox-features", sandbox_features_hash, "series",
      _snapd_json_get_string(object, "series", NULL), "store",
      _snapd_json_get_string(object, "store", NULL), "version",
      _snapd_json_get_string(object, "version", NULL), "refresh-hold",
      refresh_hold, "refresh-last", refresh_last, "refresh-next", refresh_next,
      "refresh-schedule", _snapd_json_get_string(refresh, "schedule", NULL),
      "refresh-timer", _snapd_json_get_string(refresh, "timer", NULL), NULL);
}

SnapdSnap *_snapd_json_parse_snap(JsonNode *node, GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
  ...
  SnapdConfinement confinement =
      parse_confinement(_snapd_json_get_string(object, "confinement", ""));
  ...
  const gchar *snap_status_string =
      _snapd_json_get_string(object, "status", "");
  ...
  g_autoptr(GPtrArray) categories_array =
      g_ptr_array_new_with_free_func(g_object_unref);
  for (guint i = 0; i < json_array_get_length(categories); i++) {
    ...
    if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
      g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                  "Unexpected categories type");
      return NULL;
    }

    JsonObject *c = json_node_get_object(node);
    g_autoptr(SnapdCategory) category =
        g_object_new(SNAPD_TYPE_CATEGORY, "featured",
                     _snapd_json_get_bool(c, "featured", FALSE), "name",
                     _snapd_json_get_string(c, "name", NULL), NULL);
    g_ptr_array_add(categories_array, g_steal_pointer(&category));
  }

  JsonObject *channels = _snapd_json_get_object(object, "channels");
  g_autoptr(GPtrArray) channels_array =
      g_ptr_array_new_with_free_func(g_object_unref);
  if (channels != NULL) {
    JsonObjectIter iter;
    ...
    while (json_object_iter_next(&iter, &name, &channel_node)) {
      ...
      SnapdConfinement confinement =
          parse_confinement(_snapd_json_get_string(c, "confinement", ""));
      g_autoptr(GDateTime) released_at =
          _snapd_json_get_date_time(c, "released-at", NULL);

      g_autoptr(SnapdChannel) channel = g_object_new(
          SNAPD_TYPE_CHANNEL, "confinement", confinement, "epoch",
          _snapd_json_get_string(c, "epoch", NULL), "name",
          _snapd_json_get_string(c, "channel", NULL), "released-at",
          released_at, "revision", _snapd_json_get_string(c, "revision", NULL),
          "size", _snapd_json_get_int(c, "size", 0), "version",
          _snapd_json_get_string(c, "version", NULL), NULL);
  ...
  g_autoptr(GDateTime) install_date =
      _snapd_json_get_date_time(object, "install-date", NULL);
  ...
  g_autoptr(GPtrArray) prices_array =
      g_ptr_array_new_with_free_func(g_object_unref);
  if (prices != NULL) {
    JsonObjectIter iter;
    ...
    while (json_object_iter_next(&iter, &currency, &amount_node)) {
      ...
      g_autoptr(SnapdPrice) price = g_object_new(
          SNAPD_TYPE_PRICE, "amount", json_node_get_double(amount_node),
  ...
  g_autoptr(GPtrArray) media_array =
      g_ptr_array_new_with_free_func(g_object_unref);
  for (guint i = 0; i < json_array_get_length(media); i++) {
    ...
    g_autoptr(SnapdMedia) media = g_object_new(
        SNAPD_TYPE_MEDIA, "type", _snapd_json_get_string(s, "type", NULL),
        "url", _snapd_json_get_string(s, "url", NULL), "width",
        (guint)_snapd_json_get_int(s, "width", 0), "height",
        (guint)_snapd_json_get_int(s, "height", 0), NULL);
    g_ptr_array_add(media_array, g_steal_pointer(&media));
  }

  g_autoptr(GPtrArray) screenshots_array =
      g_ptr_array_new_with_free_func(g_object_unref);

  /* The tracks field was originally incorrectly named, fixed in snapd 61ad9ed
   * (2.29.5) */
  ...
  g_autoptr(GPtrArray) links_array =
      g_ptr_array_new_with_free_func(g_object_unref);      
  GList *link_types = _snapd_json_get_keys(links_object);
  for (GList *iter = link_types; iter != NULL; iter = iter->next) {
      const gchar *key = (const gchar *)iter->data;
      JsonNode *node = json_object_get_member(links_object, key);
      if (json_node_get_node_type(node) != JSON_NODE_ARRAY) {
          g_list_free_full(link_types, g_free);
          g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                    "Unexpected url type for key '%s'", key);
          return NULL;
      }

      g_autoptr(GPtrArray) urls_array = g_ptr_array_new_with_free_func(g_free);

      JsonArray *a = _snapd_json_get_array(links_object, key);
      guint array_length = json_array_get_length(a);
      ...
      for (guint i = 0; i < array_length; i++) {
          ...
          g_ptr_array_add(urls_array, g_strdup(url));
      }
      g_autoptr(SnapdLink) link = g_object_new(SNAPD_TYPE_LINK,
                                             "type", key, 
                                             "url", (GStrv) g_ptr_array_free(g_steal_pointer(&urls_array), FALSE), 
                                             NULL);
  ...
  if (publisher != NULL) {
    publisher_display_name =
        _snapd_json_get_string(publisher, "display-name", NULL);
    publisher_id = _snapd_json_get_string(publisher, "id", NULL);
    publisher_username =
        _snapd_json_get_string(publisher, "username", publisher_username);
    const gchar *validation =
        _snapd_json_get_string(publisher, "validation", NULL);
  ...
  JsonObject *refresh_inhibit =
      _snapd_json_get_object(object, "refresh-inhibit");
  g_autoptr(GDateTime) proceed_time = NULL;
  if (refresh_inhibit != NULL)
    proceed_time =
        _snapd_json_get_date_time(refresh_inhibit, "proceed-time", NULL);

  return g_object_new(
      SNAPD_TYPE_SNAP, "apps", apps_array, "base",
      _snapd_json_get_string(object, "base", NULL), "broken",
      _snapd_json_get_string(object, "broken", NULL), "categories",
      categories_array, "channel",
      _snapd_json_get_string(object, "channel", NULL), "channels",
      channels_array, "common-ids", (GStrv)common_ids_array->pdata,
      "confinement", confinement, "contact",
      _snapd_json_get_string(object, "contact", NULL), "description",
      _snapd_json_get_string(object, "description", NULL), "devmode",
      _snapd_json_get_bool(object, "devmode", FALSE), "download-size",
      _snapd_json_get_int(object, "download-size", 0), "hold", hold, "icon",
      _snapd_json_get_string(object, "icon", NULL), "id",
      ...
      _snapd_json_get_string(object, "license", NULL), "links", links_array, "media", media_array,
      ...
      publisher_validation, "revision",
      _snapd_json_get_string(object, "revision", NULL), "screenshots",
      screenshots_array, "snap-type", snap_type, "status", snap_status,
      "store-url", _snapd_json_get_string(object, "store-url", NULL), "summary",
      _snapd_json_get_string(object, "summary", NULL), "title",
      _snapd_json_get_string(object, "title", NULL), "tracking-channel",
      _snapd_json_get_string(object, "tracking-channel", NULL), "tracks",
      (GStrv)track_array->pdata, "trymode",
      _snapd_json_get_bool(object, "trymode", FALSE), "version",
      _snapd_json_get_string(object, "version", NULL), "website",
      _snapd_json_get_string(object, "website", NULL), "proceed-time",
      proceed_time, NULL);
}

SnapdApp *_snapd_json_parse_app(JsonNode *node, const gchar *snap_name,
                                GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
                "Unexpected app type");
  ...
  return g_object_new(
      SNAPD_TYPE_APP, "name", _snapd_json_get_string(object, "name", NULL),
      "active", _snapd_json_get_bool(object, "active", FALSE), "common-id",
      _snapd_json_get_string(object, "common-id", NULL), "daemon-type",
      daemon_type, "desktop-file",
      _snapd_json_get_string(object, "desktop-file", NULL), "enabled",
      _snapd_json_get_bool(object, "enabled", FALSE), "snap",
      snap_name ? snap_name : app_snap_name, NULL);
}

SnapdAlias *_snapd_json_parse_alias(JsonNode *node, const gchar *snap_name,
  ...
  return g_object_new(SNAPD_TYPE_ALIAS, "snap", snap_name, "app-auto",
                      _snapd_json_get_string(object, "auto", NULL),
                      "app-manual",
                      _snapd_json_get_string(object, "manual", NULL), "command",
                      _snapd_json_get_string(object, "command", NULL), "name",
...
SnapdUserInformation *_snapd_json_parse_user_information(JsonNode *node,
  ...
  if (json_object_has_member(object, "macaroon")) {
    g_autoptr(JsonArray) discharges =
        _snapd_json_get_array(object, "discharges");
    ...
    auth_data =
        snapd_auth_data_new(_snapd_json_get_string(object, "macaroon", NULL),
                            (GStrv)discharge_array->pdata);
  }

  return g_object_new(
      SNAPD_TYPE_USER_INFORMATION, "id", _snapd_json_get_int(object, "id", -1),
      "username", _snapd_json_get_string(object, "username", NULL), "email",
      _snapd_json_get_string(object, "email", NULL), "ssh-keys",
      (GStrv)ssh_key_array->pdata, "auth-data", auth_data, NULL);
}

GHashTable *_snapd_json_parse_object(JsonObject *object, GError **error) {
  g_autoptr(GHashTable) attributes = g_hash_table_new_full(
  ...
  while (json_object_iter_next(&iter, &attribute_name, &node)) {
    ...
    if (variant == NULL)
      return NULL;
    g_hash_table_insert(attributes, g_strdup(attribute_name),
...
GHashTable *_snapd_json_parse_attributes(JsonNode *node, GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
...
SnapdSlot *_snapd_json_parse_slot(JsonNode *node, GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
  ...
  g_autoptr(JsonArray) connections =
      _snapd_json_get_array(object, "connections");
  g_autoptr(GPtrArray) plug_refs =
      g_ptr_array_new_with_free_func(g_object_unref);
  ...
  if (json_object_has_member(object, "attrs")) {
    attributes = _snapd_json_parse_attributes(
        json_object_get_member(object, "attrs"), error);
    if (attributes == NULL)
      return FALSE;
  } else
    attributes = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                       (GDestroyNotify)g_variant_unref);

  return g_object_new(SNAPD_TYPE_SLOT, "name",
                      _snapd_json_get_string(object, "slot", NULL), "snap",
                      _snapd_json_get_string(object, "snap", NULL), "interface",
                      _snapd_json_get_string(object, "interface", NULL),
                      "label", _snapd_json_get_string(object, "label", NULL),
                      "connections", plug_refs, "attributes", attributes,
                      // FIXME: apps
                      NULL);
}

SnapdPlug *_snapd_json_parse_plug(JsonNode *node, GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
  ...
  return g_object_new(SNAPD_TYPE_PLUG, "name",
                      _snapd_json_get_string(object, "plug", NULL), "snap",
                      _snapd_json_get_string(object, "snap", NULL), "interface",
                      _snapd_json_get_string(object, "interface", NULL),
                      "label", _snapd_json_get_string(object, "label", NULL),
                      "connections", slot_refs, "attributes", attributes,
                      // FIXME: apps
                      NULL);
}

SnapdSlotRef *_snapd_json_parse_slot_ref(JsonNode *node, GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
  ...
  return g_object_new(SNAPD_TYPE_SLOT_REF, "slot",
                      _snapd_json_get_string(object, "slot", NULL), "snap",
                      _snapd_json_get_string(object, "snap", NULL), NULL);
}

SnapdPlugRef *_snapd_json_parse_plug_ref(JsonNode *node, GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
  ...
  return g_object_new(SNAPD_TYPE_PLUG_REF, "plug",
                      _snapd_json_get_string(object, "plug", NULL), "snap",
                      _snapd_json_get_string(object, "snap", NULL), NULL);
}

SnapdConnection *_snapd_json_parse_connection(JsonNode *node, GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
  ...
  if (json_object_has_member(object, "slot")) {
    slot_ref = _snapd_json_parse_slot_ref(
        json_object_get_member(object, "slot"), error);
    if (slot_ref == NULL)
      return NULL;
  }
  g_autoptr(SnapdPlugRef) plug_ref = NULL;
  if (json_object_has_member(object, "plug")) {
    plug_ref = _snapd_json_parse_plug_ref(
        json_object_get_member(object, "plug"), error);
    if (plug_ref == NULL)
      return NULL;
  }
  ...
  if (json_object_has_member(object, "slot-attrs")) {
    slot_attributes = _snapd_json_parse_attributes(
        json_object_get_member(object, "slot-attrs"), error);
    if (slot_attributes == NULL)
      return FALSE;
  } else
    slot_attributes = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                            (GDestroyNotify)g_variant_unref);

  g_autoptr(GHashTable) plug_attributes = NULL;
  if (json_object_has_member(object, "plug-attrs")) {
    plug_attributes = _snapd_json_parse_attributes(
        json_object_get_member(object, "plug-attrs"), error);
    if (plug_attributes == NULL)
      return FALSE;
  } else
    plug_attributes = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
                                            (GDestroyNotify)g_variant_unref);

  return g_object_new(
      SNAPD_TYPE_CONNECTION, "slot", slot_ref, "plug", plug_ref, "interface",
      _snapd_json_get_string(object, "interface", NULL), "manual",
      _snapd_json_get_bool(object, "manual", FALSE), "gadget",
      _snapd_json_get_bool(object, "gadget", FALSE), "slot-attrs",
      slot_attributes, "plug-attrs", plug_attributes, NULL);
}

SnapdInterface *_snapd_json_parse_interface(JsonNode *node, GError **error) {
  if (json_node_get_value_type(node) != JSON_TYPE_OBJECT) {
    g_set_error(error, SNAPD_ERROR, SNAPD_ERROR_READ_FAILED,
  ...
  return g_object_new(SNAPD_TYPE_INTERFACE, "name",
                      _snapd_json_get_string(object, "name", NULL), "summary",
                      _snapd_json_get_string(object, "summary", NULL),
                      "doc-url",
                      _snapd_json_get_string(object, "doc-url", NULL), "plugs",
                      plug_array, "slots", slot_array, NULL);
}
