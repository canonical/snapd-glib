...
SnapdGetFind *_snapd_get_find_new(GCancellable *cancellable,
                                  ...
                                  gpointer user_data) {
  SnapdGetFind *self = SNAPD_GET_FIND(g_object_new(
      snapd_get_find_get_type(), "cancellable", cancellable, "ready-callback",
      callback, "ready-callback-data", user_data, NULL));

  return self;
}

void _snapd_get_find_set_common_id(SnapdGetFind *self, const gchar *common_id) {
  g_free(self->common_id);
  self->common_id = g_strdup(common_id);
}

void _snapd_get_find_set_query(SnapdGetFind *self, const gchar *query) {
  g_free(self->query);
  self->query = g_strdup(query);
}

void _snapd_get_find_set_name(SnapdGetFind *self, const gchar *name) {
  g_free(self->name);
  self->name = g_strdup(name);
}

void _snapd_get_find_set_select(SnapdGetFind *self, const gchar *select) {
  g_free(self->select);
  self->select = g_strdup(select);
}

void _snapd_get_find_set_section(SnapdGetFind *self, const gchar *section) {
  g_free(self->section);
  self->section = g_strdup(section);
}

void _snapd_get_find_set_category(SnapdGetFind *self, const gchar *category) {
  g_free(self->category);
  self->category = g_strdup(category);
}

void _snapd_get_find_set_scope(SnapdGetFind *self, const gchar *scope) {
  g_free(self->scope);
  self->scope = g_strdup(scope);
}

GPtrArray *_snapd_get_find_get_snaps(SnapdGetFind *self) { return self->snaps; }

const gchar *_snapd_get_find_get_suggested_currency(SnapdGetFind *self) {
  return self->suggested_currency;
}

static SoupMessage *generate_get_find_request(SnapdRequest *request,
                                              GBytes **body) {
...
static gboolean parse_get_find_response(SnapdRequest *request,
                                        guint status_code,
  ...
  self->suggested_currency =
      g_strdup(_snapd_json_get_string(response, "suggested-currency", NULL));

  return TRUE;
}

static void snapd_get_find_finalize(GObject *object) {
...
static void snapd_get_find_class_init(SnapdGetFindClass *klass) {
...
static void snapd_get_find_init(SnapdGetFind *self) {}
