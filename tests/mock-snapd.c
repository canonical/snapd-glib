...
MockLink *mock_snap_add_link(MockSnap *snap, const gchar *type,
                             const gchar *url) {
  MockLink *link = g_slice_new0(MockLink);
  link->type = g_strdup(type);

  GList *urls_list = NULL;
  urls_list = g_list_append(urls_list, g_strdup(url));
  link->urls = urls_list;
  ...
  if (snap->links != NULL) {
    ...
    for (GList *link = snap->links; link; link = link->next) {
      MockLink *urls = link->data;
        ...
        json_builder_begin_array(builder);
        for (GList *url = urls->urls; url; url = url->next) {
          json_builder_add_string_value(builder, url->data);
        }
        json_builder_end_array(builder);
    }
